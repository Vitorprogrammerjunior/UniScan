        ...
        <form action="{{ route('admin.master.usuarios.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-univc-500 focus:border-univc-500"
                       placeholder="Ex: maria@univc.edu.br">
                @error('email')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
                <div class="flex items-start">
                    ...
                    <p class="text-blue-700 text-sm">
                        O novo usuário terá os mesmos privilégios que você. Poderá cadastrar, editar e visualizar todos os patrimônios.
                    </p>
                </div>
            </div>
            ...
            <div class="flex space-x-4">
                <a href="{{ route('admin.master.usuarios.index') }}" 
                   class="flex-1 px-4 py-2 text-center text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors">
                    Cancelar
                </a>
                <button type="submit" 
