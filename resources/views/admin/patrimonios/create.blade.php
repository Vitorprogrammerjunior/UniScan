...
@section('header', 'Novo Patrimônio')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.patrimonios.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="codigo_barra" name="codigo_barra" value="{{ old('codigo_barra') }}" required
                ...
                <div>
                    ...
                    <select id="tipo_patrimonio_id" name="tipo_patrimonio_id" required
                        ...
                        @foreach($tipos as $tipo)
                            <option value="{{ $tipo->id }}" {{ old('tipo_patrimonio_id') == $tipo->id ? 'selected' : '' }}>
                                {{ $tipo->nome }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="local_armazenamento_id" name="local_armazenamento_id" required
                        ...
                        @foreach($locais as $local)
                            <option value="{{ $local->id }}" {{ old('local_armazenamento_id') == $local->id ? 'selected' : '' }}>
                                {{ $local->nome }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <select id="situacao" name="situacao" required
                        ...
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-univc-500 focus:border-univc-500">
                    <option value="disponivel" {{ old('situacao') == 'disponivel' ? 'selected' : '' }}>Disponível</option>
                    <option value="manutencao" {{ old('situacao') == 'manutencao' ? 'selected' : '' }}>Manutenção</option>
                    <option value="emprestado" {{ old('situacao') == 'emprestado' ? 'selected' : '' }}>Emprestado</option>
                    <option value="descartado" {{ old('situacao') == 'descartado' ? 'selected' : '' }}>Descartado</option>
                    <option value="separado_descarte" {{ old('situacao') == 'separado_descarte' ? 'selected' : '' }}>Separado p/ Descarte</option>
                </select>
                @error('situacao')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div x-data="{ mostrar: '{{ old('situacao') }}' === 'emprestado' }"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="local_original_id" name="local_original_id"
                            ...
                            <option value="">Selecione...</option>
                            @foreach($locais as $local)
                                <option value="{{ $local->id }}" {{ old('local_original_id') == $local->id ? 'selected' : '' }}>
                                    {{ $local->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="local_emprestado_id" name="local_emprestado_id"
                            ...
                            <option value="">Selecione...</option>
                            @foreach($locais as $local)
                                <option value="{{ $local->id }}" {{ old('local_emprestado_id') == $local->id ? 'selected' : '' }}>
                                    {{ $local->nome }}
                                </option>
                            @endforeach
                        </select>
