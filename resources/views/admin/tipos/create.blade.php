...
@section('title', 'Novo Tipo de Patrimônio')
@section('header', 'Novo Tipo de Patrimônio')

@section('content')
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.tipos.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="nome" name="nome" value="{{ old('nome') }}" required
                       ...
                       placeholder="Ex: EQUIPAMENTO, ADAPTADOR, AMPERÍMETRO">
                @error('nome')
            ...
            <div>
                ...
                <textarea id="descricao" name="descricao" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-univc-500 focus:border-univc-500"
                          placeholder="Descrição opcional do tipo...">{{ old('descricao') }}</textarea>
                @error('descricao')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
